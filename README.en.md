...
| **HAR traffic capture** | Full HTTP/S request log via browsermob-proxy (QUIC/HTTP2 disabled for complete capture) |
| **GDPR violation detection** | 12 violation categories, risk-rated HIGH / MEDIUM / LOW |
| **localStorage / sessionStorage scan** | Pre- and post-consent snapshots, flags tracking keys written after reject |
| **Cookie attribute audit** | HttpOnly, Secure, SameSite=None, expiry > 13 months (EDPB guideline) |
| **Lazy-load counter-measures** | Scroll simulation + mouse zigzag to trigger IntersectionObserver-gated resources |
| **Subpage crawling** | Selectable depth 1–5: visits internal links within the same HAR session |
| **Second-layer consent handling** | Automatically disables all toggles/switches in granular consent panels |
...
**Main window — URL input, browser & depth selection, batch mode**

<img width="1236" alt="CIAhO main window" src="https://github.com/user-attachments/assets/5b34147b-0a3f-47f6-9617-0a923244ee8c" />

</div>

<br>

| Results — Charts & Privacy Score | GDPR Violations tab |
|:---:|:---:|
| <img width="779" alt="Charts and privacy score" src="https://github.com/user-attachments/assets/5c04d1f0-bbbd-46fd-8c0d-e8dc94685b27" /> | <img width="779" alt="GDPR violations detail" src="https://github.com/user-attachments/assets/072aa1af-7154-4215-9bba-86615b09f10a" /> |
...
CIAhO automatically assigns a compliance risk level based on what is observed in network traffic, cookies, and browser storage after each consent choice:
...
| 🔴 **High** | Art. 5(1)(b) | Tracking keys written to **localStorage after reject** |
| 🔴 **High** | Art. 32 | **SameSite=None without Secure** on tracking cookie |
...
| 🟡 **Medium** | Art. 32 | Missing **HttpOnly / Secure** on tracking cookie |
| 🟢 **Low** | Art. 5(1)(c) | Excessive trackers (data minimisation) |
| 🟢 **Low** | Art. 5(1)(e) | Tracking cookie lifetime **> 13 months** (EDPB) |
...
After banner detection, all granular **toggles and switches** in second-layer consent panels are automatically flipped to OFF before clicking Reject, ensuring the most restrictive test condition.
...
Enter one or more comma-separated URLs, or click 📂 to load a `.txt` list. Choose your browser, set crawl depth (1–5), and start the scan. Results appear across multiple tabs: **Charts · Privacy Score · Summary · Cookie Details · GDPR · Fingerprinting · Ranking · JSON**.
...
|---|---|
| 1 | Main page only |
| 2 | Main page + 1 internal subpage |
| 3 | Main page + 2 subpages |
| 4 | Main page + 3 subpages |
| 5 | Main page + 4 subpages |
...
├── screenshot_accept.png  # page screenshot after Accept
├── screenshot_reject.png  # page screenshot after Reject
└── screenshot_necessary.png  # page screenshot after Necessary only
```
